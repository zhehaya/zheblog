...
题目：洛谷P3763,P3370,P3501,P4391P5270,P5537
...
2. $P[i]\ne P[j]$ ，以下图来分析，说明后缀的“ $阴影 w+P[i]$ ”与前缀的“ $阴影 w+P[j]$ ”不匹配,只能缩小范围找新的交集。把前缀向后滑动,也就是通过减小 $j$ 缩小前缀的范围,直到找到一个匹配的 $P[i]=P[j]$ 为止。如何减小 $j$?只能在 $w$ 上继续找最大交集,这个新的最大交集是 $Next[j]$ ,所以更新 $j'=Next[j]$ 。图 (b)给出了完整的子串 $P[0]\sim P[i]$ ,最后的字符 $P[i]≠P[i]$ 。斜线阴影v是w上的最大交集,下一步判断:若 $P[i]=P[j']$ ,则 $Next[i+1]$ 等于 $v$ 的长度加 1,即 $Next[j']+1$ ; 若 $P[i]≠P[j']$ ,继续更新 $P[i]≠P[j']$ 。
...
既然计算出了前缀数组，我们便可以进行较为简便的匹配,维护双指针$i$代表原串扫到的位置，$j$代表匹配串已经匹配到了哪里（$j$为匹配串长度时证明已经匹配成功，将匹配数加1），一旦下一位不再匹配或$j$等于匹配串长度，由于除了下一位前$j$位依旧匹配，所以我们不断另$j=next[j]$直到下一位可以匹配或$j$归为0时将$i$增加，继续扫描原串，按此步骤扫描完为止（可以发现和前缀数组求解步骤极其类似）。
...
完全理解了，具体来说一个节点代表一个endpos，每个节点记录的信息有：

1. link链的father
2. 实边链接的儿子
3. 这个endpos代表字符串的最长长度

其中endpos代表字符串的最短长度等于father记录的最长长度加1，不存在重复的endpos，自然一个子串只会出现在一个节点，而且endpos集合必定属于father的endpos集合，link链可以形成一个link树，实变构成的图中保证从根节点出发到这个节点的所有路径就是这个endpos包含的所有字符串。

father：最长为最短紧接的后缀

son：son包含一个字符串为加一个字符，不让son产生重复字符串，产生的字符串连续

--》一个节点的endpos包含的字符串连续》》去掉最后一个字符的依然连续--》》》去掉最后一个字符由上一个节点链接而成-》》》所以连边的所有节点是一条father链的一部分

如果多一个字符呢？原串所在的endpos节点是last，现在的下标是i

1. last出来一个儿子链接这个新字符，表示新的原串所在的节点（cur),此时cur的end信息只有一个i
2. 从last开始跳father，找到有多少产生的新字符串，这些新字符串的end信息为i
3. 假设以$s_i$结尾的子串是$As_i$，则A一定出现在last跳的father链中
4. 如果跳到一个节点t，其最长子串为B，没有$s_i$的儿子，则SAM上不存在$Bs_i$的节点，直接连边（具体来说，若存在，其一定作为t的儿子，否则B就会重复出现在两个节点上）。
5. 有$s_i$的儿子，其father链上部分边已经连边,而且其所有后缀肯定已经全部出现，这些的end都加了一个i（注意是部分边，因为再往上一些的father链可能连的也是其的father）
6. 第一个有$s_i$儿子的t，设对应儿子为j，为了给cur找father，j可能含有更长的字符串无法作为father，而这更长的字符串来自于father链下方，故需要把j拆点，拆为两点，新点继承旧点所有儿子，新点father变为旧点father,旧点father变为新点，遍历father链上方把所有儿子是这个点的节点儿子都变更（有些情况特判即可）
7. 关于特判：直接跳到根节点，连边后father就是根节点；恰好没有更长的字符串不用拆点，设置father为这个新点即可
8. 至此我们多了一个新的endpos节点并且处理好了之前所有endpos节点的关系，保证这个节点只包含新出现在这里的字符串。

完全以endpos的角度：

1. 加一个字符多出来$\{i\}$的endpos，即没有的最短后缀-》原串
2. 加一个字符会使一些endpos多出来$i$即空串-》已经有的最长后缀。走father链可以走出来
3. 但是有些endpos会分化，但是分化的只是最长后缀所在的endpos，因为其上面一些都是这个endpos的father链上的了
4. 分化就拆点
5. 受到影响的是这个endpos的父亲链上方所有点
